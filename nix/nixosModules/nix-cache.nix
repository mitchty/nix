...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # OK. Trying to get this to start after network up is a failure idea too ye
    # olde sleep works so whatever who cares.
    ...
    # nginx only used for docker container caching now
    services.nginx =
      let
      ...
      in
      {
        ...
        virtualHosts = {
          # "cachix.cachix.org.cache.home.arpa" = {
          #   listen = [
          #     {
          #       addr = "10.10.10.141";
          #       port = 80;
          ...
          #   ];
          #   extraConfig = ''
          #     set $upstream_endpoint https://cachix.cachix.org;
          #   '';
          ...
          #   };
          # };
          # "nix-community.cachix.org.cache.home.arpa" = {
          #   listen = [
          #     {
          #       addr = "10.10.10.142";
          #       port = 80;
          ...
          "${cfg.dockerCname}" = {
            ...
            locations = {
              ...
              #   '';
              # };
              # Rewrite "short" urls from say alpine:latest to
              # library/alpine:latest like the docker/podman commands.
              "~^/v2/([^/]+)(/.*)" = {
                extraConfig = ''
                  # rewrite /v2/alpine/... -> /v2/library/alpine/...
                  rewrite ^/v2/.* /v2/library/$1$2 break;
              ...
              "@handle_redirect" = {
                extraConfig = ''
                  ...
                  proxy_pass https://$redirect_host$redirect_path$is_args$redirect_args;
