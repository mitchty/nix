...
let
  ...
  # TODO: move me into lib someday?
  linuxSystem = system: builtins.elem system nixpkgs.lib.systems.doubles.linux;
      ...
      modules = [
        ({ lib
         ...
         , ...
          ...
          imports = [
            # Build on top of the minimal installer derivation, gooeys are lame
            # text4eva.
...
in
{
  ...
  perSystem = { system, ... }:
    mkIf (linuxSystem system) {
      ...
      packages.rescueIso = (rescueConfig system).config.system.build.isoImage;
    };
}
