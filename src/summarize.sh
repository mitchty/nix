...
cat whisper.out | sed 's/\x1B\[[0-9;]\{1,\}[A-Za-z]//g' | awk '/-->/ {gsub(/\\/, "", $0);$1=$2=$3=""; print}' | grep -Ev '\[(MUSIC|SOUND|BLANK_AUDIO)\]' | sed -e 's/^   //' | tee whisper.filtered
...
} | tee | ollama run "${model}" | sed -e '/<think>/,/<\/think>/d'
