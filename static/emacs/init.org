...
;; auto-insert related has to be outside emacs too for god knows why or cares I didn't debug it much
...
** Compilation mode related

TODO: testing compilation mode color shenanigans

More shenanigans to yeet from in https://www.reddit.com/r/emacs/comments/1h2o8hr/share_your_mx_compile_compilationmode_config/
...
(use-package multi-compile
  ...
  (multi-compile-alist '((rust-mode . (("cargo build" ("cargo build --workspace")
                                      (locate-dominating-file buffer-file-name ".git"))
                                     ("cargo release" "cargo build --release --workspace"
                                      (locate-dominating-file buffer-file-name ".git")))))))

;; Not sure I want to recompile every save...
;; (use-package recompile-on-save
;;     :ensure nil
;;     :defer
;;     :after fancy-compilation
;;     :init
;;     (recompile-on-save-advice compile))
#+END_SRC

The use-package emacs declaration for general configuration. Everything except ^^^ that I can't seem to make work in use-package is up there.
...
(use-package emacs
  ...
  ;; TODO figure it out future mitch
  ;; (:map prog-mode-map
  ;;       ("C-c C-c"   . compile))
  :hook
  ((prog-mode text-mode org-mode) . hl-line-mode)
  ((prog-mode text-mode org-mode) . auto-revert-mode)
...
(setq-local lisp-indent-offset 2)
  ...
  ;; ediff related, the yes or no is here cause ediff whines all the damn time and I hate typing 3 thing with it. Maybe future me figures out to let it just do it to it vs ask like a 2 year old every time.
  ...
  ;; I hate upcase asking just do it
  (put 'upcase-region 'disabled nil)

  ;; And same for delete-selection mode
  (delete-selection-mode 1)

  ;; utf-8 by default its not the 70's anymore
...
Sick of solarized, going back to good old black on white minimalism like paper. Also note auto-dark is no longer macos only but its not exactly working on linux think it needs some wonky gnome thing to work sigh...

TODO get this hunk o junk working on linux by hook or crook.
...
Setup the presets, by default troll with comic code sans cause its funny and not a bad font honestly. Plus people have a weird hatred for comic sans like fonts which just means its the best font like that saying goes: there are only two types of programming languages those nobody uses and those everyone complains about.
...
(use-package fontaine
  ...
  :config
  ;; rtx has a gihugic monitor so use smaller size stuff
  (fontaine-set-preset
     (cond ((string-equal "rtx" (system-name)) 'comic-base)
       (t 'comic-medium))))
#+END_SRC
...
All the packages I abuse for shenanigans and whatnot.

*** TODO TESTING PACKAGES

This section is only for testing crap. Though like anything temporary it can become permanent and I forget to move it on yonder out of here.

Abusing mermaid for diagrams test, note the hook at the end of config is a hack. Future mitch can fix it in post... I'm just wanting to see the tangled images inline, and for now mermaids the only thing that does it. I'm sure there is a better spot... somewhere for it.
...
Trying out eca for abominable intelligence integration. Promotable methinks as its working decently enough.
...
Not in use-package yet (ever?) https://github.com/steveyegge/efrit
...
TODO: this shits not working with ollama for some reason, future mitch figure it out, or just maybe hook eca up to ollama instead? This was ass to setup in general compared to eca.
...
;;  (chatgpt-shell-ollama-load-models)
  :config
  (setq chatgpt-shell-model-version "qwen2.5:latest")
  (setq chatgpt-shell-ollama-api-url-base "http://10.10.10.220:11434")
;;  (setq chatgpt-shell-api-url-base "http://10.10.10.220:11434")
  (setq chatgpt-shell-openai-key "")
;;  (setq chatgpt-shell-ollama-models
;;      '("qwen2.5-coder:32b"
...
OR perspective? I have no freaking clue at this point too many goddamn options.
...
(use-package perspective
  ...
  :custom
  ;; C-c M-p no bueno for xorg/plasma sigh ffs testing shared config is ass
  ;; across macos/linux
...
Eff it lets try this fork out too cause I'm a sadist I guess. Not sure if I'll keep it.
...
(use-package persp-mode
  ...
  :defer
  :custom
  ;; not sure if I want to auto load buffers or not
...
;; TODO: not entirely sure this is correct, been trying to get the battery crap
;; out of mode line when its charged mostly charged. If its below 95% and
;; (dis)charging though then I do want a display.
(defun my/battery-display-if-not-charged (original-func)
  (let ((battery-mode-line-format
	 (if (and ;; If the charge is over 95% skip display its in nukes and horseshoe or grenade territory of close enough.
	      (string-match ".*charging" (battery-format "%B" (funcall battery-status-function)))
	      (< (string-to-number (battery-format "%p" (funcall battery-status-function))) 95))
             battery-mode-line-format)))
    (funcall original-func)))

(advice-add 'battery-update :around #'my/battery-display-if-not-charged)
...
(setq-default mode-line-format
  (list "%e"
	...
	;; TODO: maybe make this constrained to things just with a battery in
	;; case I want to run in text only mode? Future me figure out past me is
	;; punting on this.
...
#+BEGIN_SRC emacs-lisp :tangle yes
(defun my-insert-default-yasnippet ()
  (when (and (bobp) (eobp))
    (yas-expand-snippet (yas-lookup-snippet "default"))))

(add-hook 'c-mode-hook 'my-insert-default-yasnippet)

(use-package yasnippet
  :ensure
  :defer
  :init
  ;;(add-to-list 'yas-snippet-dirs (expand-file-name "~/.emacs.d/yasnippet"))
...
(use-package org
  ...
  (add-to-list 'org-structure-template-alist '("mer" . "SRC mermaid :file "))
  ...
  ;;TODO autoformat elisp code blocks when exiting org babel blocks/buffers. Not sure this is working...
(define-advice org-edit-src-save (:before (&rest _args) format-elisp)
  "Run `pp-buffer' on elisp code blocks when org babel would save the buffer to the top level doc."
  (when (eq major-mode 'emacs-lisp)
    (message "Formatting elisp code buffer...")
    (pp-buffer)))
  :custom
...
*** TODO org babel ob-mermaid testing

Validate this works ok enough.

#+BEGIN_SRC emacs-lisp :tangle yes
(use-package mermaid-mode :ensure :defer)
(use-package ob-mermaid :after (org, mermaid-mode) :ensure :defer)
#+END_SRC

*** org latex/babel testing

#+BEGIN_SRC emacs-lisp :tangle no

(use-package ob-beamer :after org :ensure :defer)
#+END_SRC
...
For gooey emacs use svg images to replace some strings n such to make it all pretty like.

#+BEGIN_SRC emacs-lisp :tangle yes
(use-package svg-tag-mode
  :if (display-graphic-p)
  :after emacs
  :hook ((prog-mode org-mode) . svg-tag-mode)
  :ensure
  :defer
  :custom
  (svg-tag-tags '(("TODO" . ((lambda (tag)
				      (svg-tag-make "TODO"
						    ...
						    :inverse t
						    :radius 2))))
                  (":tangle yes" . ((lambda (tag)
				      (svg-tag-make "tangled"
						    :face 'diff-added
						    :inverse nil
                :weight 'light))))
                  (":tangle no" . ((lambda (tag) (svg-tag-make "untangled"
 						    :face 'error
                :weight 'light)))))))
#+END_SRC
...
I used to have org-bullets or org-superstar-mode but this package does more of what I would like to an org mode buffer.
...
TODO: this should be builtin to emacs 30.1 test it untangled
...
TODO eglot-booster/emacs-lsp-booster removal for 30.x+ allegedly its json parsing perf is 8x+ faster so maybe not needed anymore. Future me problem.
...
(use-package flycheck-posframe
  ...
  :custom
  (flycheck-posframe-warning-prefix "omg! ")
...
For GL Shader Language mode.

#+BEGIN_SRC emacs-lisp :tangle yes
(use-package glsl-mode :ensure :defer)
#+END_SRC

*** wgsl-mode

For WebGpu Shader Language mode.

#+BEGIN_SRC emacs-lisp :tangle yes
(use-package wgsl-mode :ensure :defer)
#+END_SRC
...
Shenanigans for/related to elisp junk.

For now just autoformatter related junk.
...
(use-package rust-mode
  ...
  :ensure
  :defer
  ;; TODO: keep? https://github.com/rust-lang/rust-mode/issues/524#issuecomment-2664699717 its not useful for emacs 30.x+
  ;; :init
