...
This is so compilation mode understands control sequences vs dumping out ^[[... junk for stuff that doesn't look at what kind of tty its on.

Except for deploy-rs which seems to ignore terminfo for its output AND also to output incorrect color control sequences to boot.
...
(use-package ansi-color
  ...
  :config
  (add-hook 'compilation-filter-hook 'my/color-compilation)
)
#+END_SRC
