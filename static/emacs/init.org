...
FUTURE MITCH DO NOT ADD *text-mode* or magit will get the minimap too probably a lot of other stuff too.
...
(use-package apheleia
  ...
  (apheleia-global-mode +1)
  :config
  (dolist (formatter '((shfmt . ("shfmt" "-ci" "-p" "-i" "2" "-bn" "-sr")))
nil)
(add-to-list #'apheleia-formatters formatter))

  (dolist (formatter '((emacs-lisp-mode . lisp-indent)
                       (rust-mode        . rustfmt))
                          nil)
    (add-to-list #'apheleia-mode-alist formatter)))

;; TODO: where the hell is this its not in melpa...
